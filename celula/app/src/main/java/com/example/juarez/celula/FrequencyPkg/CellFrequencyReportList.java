...
public class CellFrequencyReportList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.frequency_reports_cell_list_activity);
...
//        ShowList();
    ...
    private void FillReportList(){
        ...
        String query = "select distinct data from frequenciacelula where celula='" + MyActivity.CodCel() + "' and data between '" + CellFrequencyReport.cellIniDate + "' and '" + CellFrequencyReport.cellEndDate + "'";
        ...
        if (count > 4) {
            ...
            for (int i = 0; i < count; i++) {
                query = "select distinct membros.nome, frequenciacelula.present " +
                        "from frequenciacelula " +
                        "join Membros on membros.codigo=frequenciacelula.userid " +
                        "where frequenciacelula.celula='" + MyActivity.CodCel() + "' and frequenciacelula.data='" + dates.get(i).toString().substring(0, 11) + "'";
            ...
            ListView lv = (ListView) findViewById(R.id.cellReportListView);
